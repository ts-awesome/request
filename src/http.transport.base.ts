...
export interface IFetchImpl<TResponse = Response> {
  (input: RequestInfo, init?: RequestInit): Promise<TResponse>;
}

export class HttpTransportBase<TOptions extends Options, TResponse extends Response> implements IHttpTransport<TOptions, TResponse> {
  constructor(
  ...
  public async request(method: HttpMethod, uri: string, options: Options & {timeout?: number} = {}): Promise<TResponse> {
